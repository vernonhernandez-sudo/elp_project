...
export const command = new Command('push')
    ...
    const onChange = async (paths) => {
        ...
        const successMessage = intl.formatMessage({ id: "jL+kt5", defaultMessage: [{ type: 0, value: "Pushed " }, { type: 6, value: "count", options: { "=0": { value: [{ type: 0, value: "no files" }] }, one: { value: [{ type: 0, value: "one file" }] }, other: { value: [{ type: 7 }, { type: 0, value: " files" }] } }, offset: 0, pluralType: "cardinal" }, { type: 0, value: " at " }, { type: 1, value: "time" }, { type: 0, value: "." }] }, {
            count: files.length,
            time: timestamp,
        });
